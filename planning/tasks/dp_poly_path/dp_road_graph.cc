...
bool DPRoadGraph::GenerateMinCostPath(
  ...
  for (std::size_t level = 1; level < path_waypoints.size(); ++level) {
    ...
    for (size_t i = 0; i < level_points.size(); ++i) {
      ...
      graph_nodes.back().emplace_back(cur_point, nullptr);  // 将撒点的矩阵填入graph_nodes中，每个level一个list。
...
void DPRoadGraph::UpdateNode(const std::list<DPRoadGraphNode> &prev_nodes,
  ...
  for (const auto &prev_dp_node : prev_nodes) {
    ...
    const auto cost = trajectory_cost->Calculate(
        curve, prev_sl_point.s(), cur_point.s(), level, total_level) + // 每个node，都遍历上一level所有点，求最小cost
        prev_dp_node.min_cost;                                          //!! 加上了上一层相应节点的最小cost
  ...
  if (level >= 2) {
    ...
    QuinticPolynomialCurve1d curve(init_sl_point_.l(), init_dl, init_ddl,  // 并且尝试直接和init_pnt连接，cost加入比较。
...
bool DPRoadGraph::SamplePathWaypoints(
  ...
  const float total_length = std::fmin(
      init_sl_point_.s() + std::fmax(init_point.v() * 8.0, kMinSampleDistance), // 纵向撒点总长最少40m，或是8s行进距离
      reference_line_.Length());
  const auto &vehicle_config =
      common::VehicleConfigHelper::instance()->GetConfig();
  ...
  if (status->planning_state().has_pull_over() &&
    ...
    if (init_sl_point_.s() > start_point_sl.s()) { 
      ...
      points->emplace_back(level_points); //如果需要停车，采样点只有一个，即为目标停车位置
      return true;
    }
  }

  for (std::size_t i = 0; accumulated_s < total_length; ++i) { //纵向每隔20-40m撒一个点
    ...
    const double kChangeLaneDeltaL = common::math::Clamp(
        level_distance * (std::fabs(init_frenet_frame_point_.dl()) + delta_dl),  //横向撒点总宽度1.2-3.5m，数量为7个
        1.2, 3.5);
    ...
    if (reference_line_info_.IsChangeLanePath() ||
        std::fabs(init_sl_point_.l()) > kLargeDeviationL) {   // 默认撒点区域位于车道内，若为变道工况，则横向行驶区域扩大到当前位置
      ...
      if (init_sl_point_.l() > eff_left_width) {
        sample_right_boundary = std::fmax(sample_right_boundary,
                                          init_sl_point_.l() - sample_l_range); //另一边的boundary为保证撒点间隔和个数相应减小
      }
    ...
    for (size_t j = 0; j < sample_l.size(); ++j) {
      common::SLPoint sl = common::util::MakeSLPoint(s, sample_l[j]); // 撒点，外层循环为s，内层为l
      sample_layer_debug.add_sl_point()->CopyFrom(sl);
      level_points.push_back(std::move(sl));            // 存进level_points中
    }
    if (!reference_line_info_.IsChangeLanePath() && has_sidepass) {
      ...
      level_points.push_back(std::move(sl_zero));     // 加一个道路中心线的点(s, 0)
    }
