...
Status EMPlanner::PlanOnReferenceLine(
  ...
  auto speed_profile =
      GenerateInitSpeedProfile(planning_start_point, reference_line_info); // 直接用的上次规划的速度profile
  if (speed_profile.empty()) {
    speed_profile = GenerateSpeedHotStart(planning_start_point); // 如果为空，给出以当前车速匀速运动的profile，最低5m/s
  ...
  for (auto& optimizer : tasks_) { //DP_POLY_PATH_OPTIMIZER; PATH_DECIDER; DP_ST_SPEED_OPTIMIZER; SPEED_DECIDER; QP_SPLINE_ST_SPEED_OPTIMIZER
